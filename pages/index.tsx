...
import {
  Calculator, Plus, Trash2, Target, DollarSign,
...
const toNumber = (val: string): number => parseFloat(val) || 0;

const MarketplaceCalculator: React.FC = () => {
  ...
  const calculateResults = () => {
    ...
    const hargaJual = toNumber(formData.hargaJual);
    const hpp = toNumber(formData.hpp);
    const biayaMarketing = toNumber(formData.biayaMarketing);
    const biayaOperasional = toNumber(formData.biayaOperasional);
              ...
              <>
                ...
                <div className="bg-white rounded-xl shadow-lg p-6">
                  ...
                  <div className="border-t border-gray-200 pt-6">
                    <h4 className="font-semibold text-gray-800 mb-4">Breakdown Harian (30 hari)</h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-cyan-50 rounded-lg p-4 text-center">
                        <p className="text-sm text-gray-600 mb-1">Unit per Hari</p>
                        <p className="text-xl font-bold text-cyan-600">{formatNumber(Math.ceil(results.targetUnit / 30))}</p>
                      </div>
                      
                      <div className="bg-teal-50 rounded-lg p-4 text-center">
                        <p className="text-sm text-gray-600 mb-1">Omset per Hari</p>
                        <p className="text-xl font-bold text-teal-600">{formatCurrency(results.targetOmset / 30)}</p>
                      </div>
                      
                      <div className="bg-amber-50 rounded-lg p-4 text-center">
                        <p className="text-sm text-gray-600 mb-1">Ads Spend per Hari</p>
                        <p className="text-xl font-bold text-amber-600">{formatCurrency(results.totalBiayaMarketing / 30)}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-gray-200 pt-6 mt-6">
                    <h4 className="font-semibold text-gray-800 mb-4">ROAS Target</h4>
                    <div className="bg-indigo-50 rounded-lg p-6 text-center">
                      <p className="text-sm text-gray-600 mb-2">Return on Ad Spend (ROAS)</p>
                      <p className="text-3xl font-bold text-indigo-600">
                        {results.totalBiayaMarketing > 0 ? (results.targetOmset / results.totalBiayaMarketing).toFixed(2) : '0'}x
                      </p>
                      <p className="text-sm text-gray-500 mt-2">
                        Setiap Rp 1 iklan menghasilkan Rp {results.totalBiayaMarketing > 0 ? (results.targetOmset / results.totalBiayaMarketing).toFixed(2) : '0'} omset
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    <div className="border-b border-gray-200 pb-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between">
                          <span className="text-gray-600">Total Omset ({formatNumber(results.targetUnit)} unit × {formatCurrency(parseInt(formData.hargaJual))})</span>
                        ...
                        <div className="ml-4 space-y-1 bg-gray-50 rounded-lg p-3">
                          ...
                          <div className="flex justify-between text-sm font-medium border-t pt-1 mt-1">
                            <span className="text-gray-700">Total Biaya Admin</span>
                            <span className="text-red-600">-{formatCurrency(results.totalBiayaAdmin)}</span>
                          </div>
                        </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between">
                          <span className="text-gray-600">Total HPP ({formatNumber(results.targetUnit)} unit × {formatCurrency(parseInt(formData.hpp))})</span>
                          <span className="font-medium text-red-600">-{formatCurrency(results.totalHPP)}</span>
                        </div>
                        
                        <div className="flex justify-between">
                          <span className="text-gray-600">Total Biaya Marketing ({formatNumber(results.targetUnit)} unit × {formatCurrency(parseInt(formData.biayaMarketing))})</span>
                          <span className="font-medium text-red-600">-{formatCurrency(results.totalBiayaMarketing)}</span>
                        </div>
                        
                        <div className="flex justify-between">
                          <span className="text-gray-600">Total Biaya Operasional ({formatNumber(results.targetUnit)} unit × {formatCurrency(parseInt(formData.biayaOperasional))})</span>
